    ...
    $.fn.blockPlugin = function(options){
									...
									buttons: [
										{
											...
											click: function() {
												...
												var type = $( this ).find('form').find('input[name="type"]:checked').val(),
														newId=Math.uuidFast(),
														newBlock = $("<div></div>").attr("type",type).css("height", self.height()-10+"px").css("width", self.width()-10+"px").css("position","relative");
        ...
        var defaultOptions = {
			...
			pasteElement: function(pastedElement, futureParent, buffer){
				var idn=Math.uuidFast(), top=options.makeNewTopForCurent(0,futureParent), child_level=parseInt(futureParent.attr('child_level'));
