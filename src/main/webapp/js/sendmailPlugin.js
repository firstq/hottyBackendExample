(function( $ ){
    $.fn.sendmailPlugin = function(options){
		...
		defaultOptions = {
			restUrl: "/sendmailblocks/",
			defaultMailFrom: "no_reply@technoserv.com",
			...
			captchaImage: $('<img src="/captcha/captcha.html" style="height: 34px"/>'),
			...
			mailForm: $('<div class="row">'+
						...
						'<form class="navbar-form" enctype="multipart/form-data">'+
						   ...
						   '<textarea name="targetMessage" class="form-control" style="margin: 0px; width: 99%;; height: 400px"></textarea><br/><br/>'+
						'</form></div></div>'),
			thisBlock : $(this),
			resizable : true,
			//popupWidth: 600,
		...
		options.mailForm.find('input[name="sendButton"]').click(function(){
			...
			$.ajax({
				...
				success: function( resp ){
					...
					options.captchaImage.fadeOut('fast').attr("src", "/captcha/captcha.html?"+new Date().getTime()).fadeIn('fast');
				},
				error: function( resp ){
