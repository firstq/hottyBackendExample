(function( $ ){
    $.fn.menuPlugin = function(options){
		var generateIdForMenu = function(min, max){
...
//								element.find('a').attr("hover-style",JSON.stringify(options.navlia_hover));
			...
			prepareElement : function(elem,v){
				...
				|| (typeof(preparedElement.attr("conected-slider-child"))!="undefined" && preparedElement.attr("conected-slider-child")!='null' && preparedElement.attr("conected-slider-child")!="") ){
					var sliderChild = $('#'+(v.dependSliderBlock || parseInt(preparedElement.attr("conected-slider-child")))),
									...
									buttons: [
										{
											...
											click: function() {
												...
												//Навес функционала в случае выбора скрола
												...
												if($('#menuItemBlock'+v.id +'Popup').find('form').find('input[name="scroll"]').is(':checked')) setSlideTo(currentElement, currentSlider, menuItemSliderChild.prop('selectedIndex'));
//												preparedElement.unbind( "click" ).attr("conected-slider-child","");
					...
					items: [
						...
						null,
						{
							...
							click: function(){
								...
								//blockOptions.i++;
								var idn = generateIdForMenu(1,999999);
								var prepareMenuBlockClone = function(menuBlockClone){
									...
									menuBlockClone.children('ul').children('li').each(function(){ //Перебираем все li
										$(this).attr("id",generateIdForMenu(1,999999)); //генерим каждому пункту меню новый id
						...
						buttons: [
							{
								...
								click: function() {
									var selectedIndex = $(this).find('form').find('select[name="menuItemParent"]').prop('selectedIndex'),
										...
										newId=generateIdForMenu(1,999999);
//									blockOptions.i++;
